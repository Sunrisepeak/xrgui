...
protected:
	void tooltip_on_drop_behavior_impl() override{
		...
		if(!rotate_animator.is_tobe_idle()){
			rotate_animator.set_target(false);
...
public:
	...
	void arrow_flip(this S& self, EnterFn&& onEnter, ExitFn&& onExit){
		if(!self.rotate_animator.is_animating()) util::update_insert(self, update_channel::draw);
		if(self.rotate_animator.is_tobe_idle()){
			self.rotate_animator.set_target(true);
			std::invoke(std::forward<EnterFn>(onEnter), self);
		} else{
			self.rotate_animator.set_target(false);
	...
	void arrow_enter(this S& self, EnterFn&& onEnter){
		if(!self.rotate_animator.is_tobe_active()) util::update_insert(self, update_channel::draw);
		self.rotate_animator.set_target(true);
		std::invoke(std::forward<EnterFn>(onEnter), self);
	}
	...
	void arrow_exit(this S& self, ExitFn&& onExit){
		if(!self.rotate_animator.is_tobe_idle()) util::update_insert(self, update_channel::draw);
		self.rotate_animator.set_target(true);
	...
	void record_draw_layer(draw_recorder& call_stack_builder) const override{
		...
		call_stack_builder.push_call_noop(*this, [](const arrow_button& s, const draw_call_param& p,
